...
class Request extends Api_Controller {
	...
	//	exit;
		...
		$certi = $this->input->get_post('matrix_certi',true);//证书名，32位
		$timestamp = $this->input->get_post('matrix_timestamp',true); //时间戳
		$token = $this->input->get_post('matrix_token',true); //验证码 md5(证书名加密匙加时间戳)
		//验证请求
		$this->load->model('certi_model');
		$check = $this->certi_model->check($certi,$timestamp,$token);
		if ($check == false) {
		//	die('{"res": "fail", "msg_id": "", "rsp": "e00093", "err_msg": "sign error", "data": "sign error"}');
		}
		
		$this->load->model('stream_model');
		$stream_id = $this->stream_model->log_first($this->input->get_post(NULL));
		
		if ($this->input->get_post('method',true)) {
			$method_name = $this->input->get_post('method',true);
			...
			foreach ($filenames as $key=>$value) {
				if ($method_name.'.php' == $value) {
					...
					//记录数据
					$this->stream_model->log_second($data,$stream_id);
					//转发
					$this->load->library('common/httpclient');
					$post_data = $this->httpclient->post('http://mosrerp.pinzhen365.com/get.php',$data['response_data']);
					var_dump($post_data);
				}
			}
